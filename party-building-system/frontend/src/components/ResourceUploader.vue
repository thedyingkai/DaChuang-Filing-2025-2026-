        ...
        this.files.forEach(item => {
          if (item.uid === options.file.uid) {
            if (response && response.code === '200') {
              const fileUrl = resolveFileUrl(response.data);
              item.url = fileUrl;
              const uid = this.user.uid != null ? this.user.uid : this.user.id;
              if (this.acid == null || this.acid === '') {
                this.$message.error('请先选择要上传到的活动');
                return;
              }
              let file = {
                uid: uid,
              ...
              this.$request.post('/resource/add', file).then(
                ...
                this.$message.error('资源入库失败，请查看后端日志或数据库 resource_view');
              });
            } else {
              item.error = 'failed';
