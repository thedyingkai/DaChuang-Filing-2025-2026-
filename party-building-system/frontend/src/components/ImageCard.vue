...
import {
  activityCoverUrl,
  ACTIVITY_COVER_PLACEHOLDER,
  activityCoverErrorFallbacks,
} from "@/utils/fileUrl";
...
export default {
  ...
  computed: {
    displayUrl() {
      return activityCoverUrl(this.url);
    },
  },
  watch: {
    url() {
      this.coverFallbackTried = [];
    },
  },
  created() {
    this.checkFileType();
  ...
  methods: {
    onImgError(e) {
      ...
      if (!img) return;
      const next = activityCoverErrorFallbacks().find(
          (u) => u && !this.coverFallbackTried.includes(u)
      );
      if (next) {
        this.coverFallbackTried.push(next);
        img.src = next;
        return;
      }
      if (img.src === ACTIVITY_COVER_PLACEHOLDER) return;
      img.onerror = null;
      img.src = ACTIVITY_COVER_PLACEHOLDER;
    },
    handleClick() {
      if (this.showType === 'image-card') {
        ...
        if (this.showType !== 'image-card') {
          this.loading = true;
          const fileExtension = this.displayUrl.split('.').pop().toLowerCase();
    ...
    previewImage() {
      return new Promise((resolve) => {
        ...
        imageWindow.document.write(`<img src="${this.displayUrl}" style="width: 100%; height: 100%; object-fit: contain;">`);
        resolve();
      });
    },
    previewVideo() {
      return new Promise((resolve) => {
        ...
        videoWindow.document.write(`<video src="${this.displayUrl}" controls style="width: 100%; height: 100%;">Your browser does not support the video tag.</video>`);
        resolve();
      });
    },
    previewPDF() {
      return new Promise((resolve) => {
        ...
        pdfWindow.document.write(`<iframe src="${this.displayUrl}" style="width: 100%; height: 100%;"></iframe>`);
        resolve();
      });
    },
    downloadFile() {
      const fileName = this.displayUrl.split('/').pop();
    ...
    checkFileType() {
      const fileType = this.displayUrl.split('.').pop().toLowerCase();
