    ...
    <el-container>
      ...
      <el-dialog
        ...
        <div style="margin: 10px 0; display:flex;flex-direction: column;justify-content: center;align-items: center;">
          ...
          <resource-uploader :acid="targetActivityId" @uploaded="onUploaderSuccess"></resource-uploader>
        </div>
      </el-dialog>
      <el-dialog
...
export default {
  ...
  methods: {
    ...
    openFile(content) {
      window.open(resolveFileUrl(content), '_blank');
    },
    onUploaderSuccess(acid) {
      if (acid != null && !this.isMenu) {
        this.getResourceFromMenu(acid);
      } else {
        this.showActivities();
      }
      this.$emit('uploaded');
    ...
    handleFileUpload(response, file, fileList) {
      ...
      if (response.code === '200') {
        this.file = {
          ...
          type: file.raw.type,
          name: file.name,
          content: resolveFileUrl(response.data),
        }
        ...
        const uid = this.user.uid != null ? this.user.uid : this.user.id;
        this.file.uid = uid;
        if (this.targetActivityId == null || this.targetActivityId === '') {
          this.$message.error('请选择要上传到的活动');
          return;
        }
        this.file.acid = this.targetActivityId;
        this.$request.post('/resource/add', this.file).then(
            res => {
              if (res && (res.code === '200' || res.code === 200)) {
                ...
                this.$message.error((res && res.msg) || '保存到数据库失败');
              }
            }
        ).catch((error) => {
          console.error('资源入库失败:', error);
          this.$message.error('资源入库失败');
        });
