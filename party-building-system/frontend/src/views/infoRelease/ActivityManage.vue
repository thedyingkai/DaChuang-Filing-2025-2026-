        ...
        <div>
          ...
          <div style="display: flex;flex-direction: column;align-items: center;margin-top: 10px;">
            <div class="image-with-overlay-container">
              <el-image :src="coverDisplayUrl" style="max-height: 200px;" fit="contain">
                <img
                    slot="error"
                    :src="defaultActivityCoverStatic"
                    alt="默认活动封面"
    ...
    <el-dialog
      ...
      <div style="margin: 10px 0; display:flex;flex-direction: column;justify-content: center;align-items: center;">
        <activity-form
            ...
            :add-type="dialogTitle === '新增活动'"
            :dialog-title="dialogTitle"
            :gid="user.gid"
            :old-data="editActivitySnapshot"
            @ActivityAdded="handleClose"
        />
      </div>
    </el-dialog>
  </div>
...
import { activityCoverUrl, DEFAULT_ACTIVITY_COVER_STATIC } from "@/utils/fileUrl";
...
export default {
  ...
  data() {
    return {
      ...
      loading: true,
      currentActivity: {},
      /** 传给 ActivityForm：新增为空对象，修改为当前活动快照（避免引用不变导致子组件不刷新） */
  ...
  computed: {
    ...
    coverDisplayUrl() {
      return activityCoverUrl(this.currentActivity && this.currentActivity.cover_image);
    },
    defaultActivityCoverStatic() {
  ...
  methods: {
    ...
    addActivity() {
      ...
      this.editActivitySnapshot = {};
      this.dialogVisible_add = true;
    },
    editActivity() {
      this.dialogTitle = '修改活动信息';
      this.editActivitySnapshot = this.currentActivity && this.currentActivity.id != null
          ? { ...this.currentActivity }
          : {};
      this.dialogVisible_add = true;
    },
