...
export default {
  ...
  methods: {
    check(tableIndex, row) {
      this.auditList = [];
      this.dialogVisible_check = true;
      // selectSubByUid 中 submit_view.id 映射为 srid；误传 row.id 会变成草稿 id，接口查不到审核记录与意见
      const submitId = row.srid != null && row.srid !== '' ? row.srid : row.id;
      const rowSendTime = row.send_time || row.save_time;
      this.$request.get('/audit/selectBySubmitId/' + submitId).then(
          res => {
            let send = {
              content: '提交',
              timestamp: rowSendTime,
            }
            ...
            if (res.code === '200' && res.data) {
              ...
              if (this.auditList.length === 0 ||
                  this.auditList[this.auditList.length - 1].timestamp < rowSendTime) {
                let latest = {
                  content: '提交，待审核',
                  timestamp: rowSendTime,
                }
    ...
    fetchData() {
      ...
      const uid = this.user.uid != null && this.user.uid !== '' ? this.user.uid : this.user.id;
